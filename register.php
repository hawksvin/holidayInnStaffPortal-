...
if($_POST && isset($_SESSION['token']) && ($_SESSION['token'] == $_POST['token']) && $set->register && $captcha) {
  ...
  $regDatenow = $now->format('Y-m-d');
  
  $Year = date('Y', strtotime($regDatenow));
  ...
  if(!isset($page->error)){
    ...
    if($set->email_validation == 1) {
      ...
      $msg = "Hello ".$options->html($name).",<br> Thank you for choosing to be a member of HolidayInn Staff community.<br/><br/> To confirm your account <a href='$link'>click here</a>.<br>If you can't access copy this to your browser<br/>$link  <br><br>Regards<br><small>Note: Dont reply to this email. If you got this email by mistake then ignore this email.</small>";
    ...
    if(($db->query("INSERT INTO `".MLS_PREFIX."users` SET ?u", $user_data)) && ($id = $db->insertId()) && $db->query("INSERT INTO `".MLS_PREFIX."privacy` SET `userid` = ?i", $id) && $db->query("INSERT INTO hi_balances (Year,userid,BalCL,BalEL,BalRH,BalML) VALUES ('$Year','$id','$BalCL','$BalEL','$BalRH','$BalML')")) {
      $page->success = 1;
